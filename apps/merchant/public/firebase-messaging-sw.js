...
messaging.onBackgroundMessage((payload) => {
  const title = payload.notification?.title || payload.data?.title || 'Pesanan Baru Masuk'
  const options = {
    body: payload.notification?.body || payload.data?.body || 'Ada pesanan baru di toko Anda.',
    icon: '/merchant/icons/merchant-192.png',
    badge: '/merchant/icons/merchant-192.png',
    tag: payload.data?.order_id ? `order-${payload.data.order_id}` : 'merchant-new-order',
    data: {
      url: payload.data?.url || '/merchant/',
...
self.addEventListener('notificationclick', (event) => {
  ...
  const rawUrl = event.notification.data?.url || '/merchant/'
