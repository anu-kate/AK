<template>
  <div class="bg-white p-4 md:p-6 rounded-3xl shadow-sm border border-gray-100 space-y-4 overflow-hidden w-full relative">
    ...
    <div v-if="activeSubTab === 'rekrutmen'" class="space-y-4 animate-fade-in w-full">
      <div class="bg-gray-50 p-4 md:p-5 rounded-2xl border border-gray-100">
        <h3 class="font-extrabold text-gray-900 mb-1 text-sm flex items-center gap-2">
          <svg class="w-5 h-5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
          Status Pendaftaran Kurir
        </h3>
        <p class="text-[11px] text-gray-400 mb-5">Kontrol apakah publik bisa mendaftar sebagai kurir baru melalui aplikasi driver.</p>

        <div class="flex items-center justify-between bg-white border border-gray-200 rounded-2xl px-4 py-4">
          <div>
            <p class="text-sm font-bold text-gray-800">Pendaftaran Kurir Baru</p>
            <p class="text-[11px] text-gray-400 mt-0.5">
              Status saat ini:
              <span :class="settings.is_driver_reg_open ? 'text-green-600 font-extrabold' : 'text-red-500 font-extrabold'">
                {{ settings.is_driver_reg_open ? '🟢 DIBUKA' : '🔴 DITUTUP' }}
              </span>
            </p>
          </div>
          <button
            type="button"
            @click="settings.is_driver_reg_open = !settings.is_driver_reg_open"
            :class="settings.is_driver_reg_open ? 'bg-green-500' : 'bg-gray-300'"
            class="relative inline-flex h-7 w-12 items-center rounded-full transition-colors duration-200 focus:outline-none"
          >
            <span
              :class="settings.is_driver_reg_open ? 'translate-x-6' : 'translate-x-1'"
              class="inline-block h-5 w-5 transform rounded-full bg-white shadow-md transition-transform duration-200"
            />
          </button>
        </div>

        <div v-if="!settings.is_driver_reg_open" class="mt-3 flex items-start gap-2.5 bg-red-50 border border-red-100 rounded-xl px-3.5 py-3 animate-fade-in">
          <svg class="w-4 h-4 text-red-400 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          <p class="text-xs text-red-600 leading-relaxed">Pendaftaran kurir sedang <strong>ditutup</strong>. Calon kurir tidak bisa mendaftar melalui aplikasi driver.</p>
        </div>
      </div>

      <button @click="saveSettings" :disabled="saving" class="w-full px-4 py-3.5 bg-blue-600 text-white font-extrabold rounded-2xl hover:bg-blue-700 shadow-md transition disabled:opacity-50 text-sm">
        {{ saving ? 'Menyimpan...' : 'Simpan Pengaturan Kurir' }}
      </button>
    </div>
