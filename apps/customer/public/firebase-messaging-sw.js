...
messaging.onBackgroundMessage((payload) => {
  const title = payload.notification?.title || payload.data?.title || 'Notifikasi Anturun'
  const options = {
    body: payload.notification?.body || payload.data?.body || 'Ada pembaruan untuk Anda.',
    icon: '/customer/icons/user-192.png',
    badge: '/customer/icons/user-192.png',
    tag: payload.data?.order_id ? `order-${payload.data.order_id}` : 'customer-notification',
    data: {
      url: payload.data?.url || '/customer/',
...
self.addEventListener('notificationclick', (event) => {
  ...
  const rawUrl = event.notification.data?.url || '/customer/'
