  ...
  <div class="space-y-6 animate-fade-in relative">
    <div class="flex bg-white border border-gray-300 p-1 rounded-xl">
      <button 
        ...
        @click="openBursa" 
        :class="[
          orderanSubTab === 'bursa' ? 'bg-blue-600 shadow-sm text-white font-bold' : 'text-gray-500 hover:text-gray-700',
          !!activeOrder ? 'opacity-50 cursor-not-allowed bg-gray-100 !text-gray-400' : ''
        ]" 
        class="flex-1 py-2.5 rounded-lg font-bold text-xs flex items-center justify-center gap-1 transition-all duration-200">
      ...
      <button 
        @click="openAktif" 
        :class="orderanSubTab === 'aktif' ? 'bg-blue-600 shadow-sm text-white font-bold' : 'text-gray-500 hover:text-gray-700'" 
        class="flex-1 py-2.5 rounded-lg font-bold text-xs flex items-center justify-center gap-1 transition-all duration-200">
    ...
    <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center justify-between gap-3">
      ...
      <div class="text-right">
        ...
        <button @click="fetchDriverCredit" class="text-[10px] font-bold text-blue-500 hover:text-blue-700">Refresh saldo</button>
      </div>
    </div>

    <button
      @click="requestNotificationPermission"
      class="w-full bg-orange-50 text-orange-700 border border-orange-100 rounded-2xl px-4 py-3 text-xs font-black flex items-center justify-center gap-2 hover:bg-orange-100 transition"
    >
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6 6 0 10-12 0v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0a3 3 0 11-6 0m6 0H9"></path></svg>
      {{ notificationPermission === 'granted' ? 'Refresh Notifikasi Orderan' : 'Aktifkan Notifikasi Orderan' }}
    </button>

    <div v-show="orderanSubTab === 'bursa'" class="animate-slide-up space-y-4">
      ...
      <div v-else class="space-y-3">
        <div v-for="order in availableOrders" :key="order.id" class="bg-white rounded-2xl border border-gray-300 shadow-sm transition-all duration-300 overflow-hidden">
          <div @click="toggleExpand(order.id)" class="p-4 cursor-pointer hover:bg-gray-50 transition">
            ...
            <div class="flex justify-between items-center mt-3">
              <div class="flex items-center gap-1.5 text-xs font-semibold text-gray-600 bg-gray-100 px-2 py-1 rounded-lg">
          ...
          <div v-show="expandedOrderId === order.id" class="px-4 pb-4 pt-3 border-t border-gray-300 bg-gray-50 animate-fade-in">
            <p class="text-[10px] text-gray-500 font-bold uppercase tracking-wide mb-2">Detail Rute Berkendara</p>
            <div class="mb-4">
              <div class="bg-white p-3 rounded-lg border border-gray-300 shadow-sm">
                <p class="text-[10px] text-gray-500 font-bold uppercase tracking-wide">Toko ke Tujuan</p>
                <p class="font-black text-gray-800 text-base mt-1">
                  {{ getRouteDisplay(order.id, 'toCustomer', 'Koordinat tujuan belum tersedia') }}
                </p>
              </div>
            </div>
    ...
    <div v-show="orderanSubTab === 'aktif'" class="animate-slide-up space-y-4">
      ...
      <div v-else class="bg-white p-5 rounded-2xl border border-blue-200 shadow-sm space-y-3 transition-colors">
        ...
        <div class="flex gap-2 mt-4 pt-4 border-t border-gray-300">
          <button v-if="['diproses', 'pending', 'siap_diambil'].includes(activeOrder.status)" @click="confirmMulaiAntar(activeOrder.id)" class="flex-1 bg-yellow-500 text-white py-2.5 rounded-xl font-bold shadow-sm hover:bg-yellow-600 transition">Mulai Antar</button>
          
          <button v-if="activeOrder.status === 'diantar'" @click="confirmSelesai(activeOrder.id)" class="flex-1 bg-green-600 text-white py-2.5 rounded-xl font-bold shadow-sm hover:bg-green-700 transition">Selesai</button>
          
          <button @click="openMerchantMaps(activeOrder.merchant)" class="px-3 bg-gray-50 text-gray-700 rounded-xl flex items-center justify-center gap-1.5 hover:bg-gray-200 transition text-xs font-bold">
            <svg class="w-4 h-4 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V10l-2.5-4h-9L5 10v11M3 10h18M5 21h14"></path></svg>
            Toko
          </button>

          <button @click="openCustomerMaps(activeOrder)" class="px-3 bg-gray-50 text-gray-700 rounded-xl flex items-center justify-center gap-1.5 hover:bg-gray-200 transition text-xs font-bold">
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-slide-up border border-gray-100 relative">
          ...
          <div v-if="(modal.actionType === 'ambil' || modal.actionType === 'selesai') && modalOrder" class="mb-6 rounded-2xl border border-gray-200 bg-gray-50 p-4 text-center">
            ...
            <p class="text-[10px] font-bold mt-1" :class="hasEnoughCredit(modalOrder) ? 'text-blue-600' : 'text-red-600'">Saldo sekarang: {{ driverCreditBalance }} AK</p>
          </div>
...
const buyAkWhatsAppLink = computed(() => {
  ...
  const message = `Halo Admin, saya ${userName} (${phone}). Saya ingin Top up Antur Kredit (AK). Saldo AK saya saat ini ${driverCreditBalance.value} AK. Mohon metode pembayarannya.`
...
const confirmSelesai = (orderId) => {
  modal.value = {
    ...
    message: 'Sahkan jika makanan atau barang telah berhasil dihantar kepada pelanggan.',
